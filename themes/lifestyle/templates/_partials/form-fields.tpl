				...
				{block name='form_field_item_checkbox'}
					<div class="checkbox">
						<span class="custom-checkbox">
							<input id="{$field.name}-checkbox" name="{$field.name}" type="checkbox" value="1" {if $field.value}checked="checked"{/if} {if $field.required}required{/if}>
							<span class="custom-checkbox-checked"><i class="fa fa-check checkbox-checked"></i></span>
							<label for="{$field.name}-checkbox">{$field.label nofilter}</label >
						</span>
					</div>
				{/block}
			...
			{elseif $field.type === 'date'}
				...
				{block name='form_field_item_date'}
					...
					{if isset($field.availableValues.comment)}
						<span class="no-padding form-control-comment">
